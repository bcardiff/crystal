...
class Crystal::CodeGenVisitor
  ...
  private def codegen_cond_impl(type : MixedUnionType)
    ...
    if has_nil || has_bool || has_pointer
      ...
      if has_bool
        value = load(bit_cast union_value(@last, type, @program.bool), llvm_context.int1.pointer)
      ...
      if has_pointer
        union_types.each do |union_type|
          ...
          pointer_value = load(bit_cast union_value(@last, type, union_type), llvm_type(union_type).pointer)
