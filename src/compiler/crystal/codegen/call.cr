...
class Crystal::CodeGenVisitor
  ...
  def codegen_direct_abi_call(call_arg, abi_arg_type)
    if cast = abi_arg_type.cast
      ...
      codegen_llvm_memcpy(final_value_casted, gep_call_arg, int32(size), int32(align), int1(0))
      ...
      if type.no_return?
        ...
        when LLVM::ABI::ArgKind::Direct
          if cast = abi_return.cast
            ...
            codegen_llvm_memcpy(final_value_casted, cast2, int32(size), int32(align), int1(0))
            @last = final_value
          end
        when LLVM::ABI::ArgKind::Indirect
