...
class Crystal::CodeGenVisitor
  def codegen_llvm_memcpy(dest : LLVM::Value, src : LLVM::Value, len : LLVM::Value, align : LLVM::Value, is_volatile : LLVM::Value)
    if LibLLVM::IS_LT_70
      ...
      if @program.has_flag?("x86_64")
        llvm_fun = get_llvm_fun("llvm.memcpy.p0i8.p0i8.i64",
          [llvm_context.void_pointer, llvm_context.void_pointer, llvm_context.int64, llvm_context.int1],
          llvm_context.void)
      ...
      align = align.const_int_get_zext_value.to_u64

      res = builder.call(llvm_fun, [dest, src, len, is_volatile])
      res.add_instruction_align_attribute(1, align, llvm_context)
      res.add_instruction_align_attribute(2, align, llvm_context)
      res
    end
  end
  ...
  def codegen_llvm_memset(dest : LLVM::Value, val : LLVM::Value, len : LLVM::Value, align : LLVM::Value, is_volatile : LLVM::Value)
    if LibLLVM::IS_LT_70
      ...
      if @program.has_flag?("x86_64")
        llvm_fun = get_llvm_fun("llvm.memset.p0i8.i64",
          [llvm_context.void_pointer, llvm_context.int8, llvm_context.int64, llvm_context.int1],
          llvm_context.void)
      ...
      res = builder.call(llvm_fun, [dest, val, len, is_volatile])
      res.add_instruction_align_attribute(1, align, llvm_context)
      res
    end
  end
